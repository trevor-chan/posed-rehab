...
if __name__ == '__main__':
    ...
    parser.add_argument('--savepath', help='Optional. Path to directory for saved video files to write to.', type=str, default='')
    ...
    #-----------------add in save variables--------------------
    save_poses = []
    wireframe_path = args.savepath+"/"+os.path.splitext(os.path.split(args.video)[1])[0]+"_wireframe.avi"
    overlay_path = args.savepath+"/"+os.path.splitext(os.path.split(args.video)[1])[0]+"_overlay.avi"
    wireframe = cv2.VideoWriter(wireframe_path, cv2.VideoWriter_fourcc('M','J','P','G'), 10, (1280,720))
    first = 1
    ...
    for i,frame in enumerate(frame_provider):
        ...
        if first == 1:
            overlay = cv2.VideoWriter(overlay_path, cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame.shape[1],frame.shape[0]))
            first = 0
        ...
        #Save poses structures to array:
        save_poses.append((poses_2d, poses_3d))
        ...
        wireframe.write(canvas_3d) #save frame to video wireframe
        ...
        overlay.write(frame) #save frame to video overlay
    ...
    wireframe.release()
    overlay.release()
    pickle.dump(save_poses, open( args.savepath+"/"+os.path.splitext(os.path.split(args.video)[1])[0]+"_saved_poses.p", "wb" ))
